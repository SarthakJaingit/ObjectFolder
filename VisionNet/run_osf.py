...
def render_path(render_poses, hwf, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0, save_extras=False,
        ...
        render_end=None, render_test = False):
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        if not(render_start <= i < render_end):
            continue
        print(i, time.time() - t)
        t = time.time()
        ...
        if render_test:
            ...
            rgb, disp, acc, extras = render(
                H, W, focal, chunk=chunk, c2w=c2w, **render_kwargs)
...
def train():
    ...
    if args.dataset_type == 'llff':
        ...
        images, poses, render_poses, hwf, i_split, metadata, render_metadata = load_osf_data(
            args.datadir, args.near, args.far, args.spiral_radius, args.half_res, args.testskip,
            args.n_render_spiral, args.render_spiral_angles)
    ...
    if args.render_only:
        ...
        with torch.no_grad():
            ...
            #testsavedir = os.path.join(basedir, expname, 'renderonly_{}_{:06d}'.format('demo2.6' if args.render_test else 'path', start))
            ...
            rgbs, _ = render_path(render_poses, hwf, args.chunk, render_kwargs_test,
                                  ...
                                  render_end=args.render_end, render_test=args.render_test)
    ...
    for i in trange(start, N_iters):
        ...
        if use_batching:
            ...
            batch = tf.transpose(batch, [1, 0, 2])  # [ro+rd+ri+rgb, B, 3]
