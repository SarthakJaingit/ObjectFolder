...
#python trainBunchSubmitGoogle.py --data_root /viscam/u/rhgao/datasets/ObjectFiles/GoogleScannedObjects/ --data_csv /viscam/u/rhgao/datasets/ObjectFiles/googleScannedObjects.csv --num_of_jobs_to_submit 2
...
def main():
	...
	for line in data_csv_file:
		...
		cmd = 'python main.py' +  \
			...
			' --checkpoints_dir ' + os.path.join(args.data_root, model_name) + \
			' --gpu_ids 0,1,2,3' + \
			' --display_freq 100' + \
			' --batchSize 50000' + \
			' --iterations 300000' + \
	...
	for sub_split in job_splits:
		...
		slurm_file.write('#SBATCH --mem=240G\n')
		...
		slurm_file.write('#SBATCH --gres=gpu:4\n')
